...
from fire_config import FINAL_FIELDS
from fire_config import state_abbr
from PATHS import SCRATCH_GDB, FINAL_PERIMETERS
...
fields_perimeters = list(FINAL_FIELDS.keys())
...
with arcpy.da.UpdateCursor(out_dissolve, [oid_field, "SHAPE@", "n_Year", "n_StartMonth", "n_StartDay", "n_Fire_ID"]) as cursor:
    for row in cursor:
        ...
        # Build ID
        date_str = f"{state_abbr}{lat[:6]}{lon[:5]}{year}{month}{day}"
